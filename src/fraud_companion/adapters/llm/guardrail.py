...
def build_tool_guardrail(allowed: frozenset[str] = ALLOWED_TOOLS) -> _WrapToolCall:
    """Build a ``wrap_tool_call`` interceptor enforcing ``allowed``.

    Pass the returned callable as ``ToolNode(..., wrap_tool_call=...)``.
    Any tool call whose name is not in ``allowed`` is rejected with
    ``DisallowedToolError`` BEFORE ``execute`` runs. Allowed tool calls
    are delegated to ``execute`` unchanged. Defaults to
    ``fraud_companion.domain.tools_spec.ALLOWED_TOOLS`` (the case-analyst
    set) so every existing call-site is unchanged; pass
